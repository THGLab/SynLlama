...
from synllama.chem.base import Drawable
...
class Reaction(Drawable):
    ...
    def __call__(self, reactants: Sequence[Molecule] | Sequence[str]) -> list[Molecule]:
        if isinstance(reactants, Sequence) and not isinstance(reactants, str):
            reactants = [Molecule.from_rdmol(m._rdmol) for m in reactants]
